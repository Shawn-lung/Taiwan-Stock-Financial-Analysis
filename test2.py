...
class DCFModel:
    def __init__(self, stock_code, short_term_growth_rate=0.10, perpetual_growth_rate=0.025):
        """
        初始化 DCF 模型
        :param stock_code: 股票代号
        :param short_term_growth_rate: 前 5 年的增长率 (默认 10%)
        :param perpetual_growth_rate: 永续增长率 (默认 2.5%)
        """
        self.stock_code = stock_code
        self.wacc_model = Wacc.WACCModel(stock_code)
        self.short_term_growth_rate = short_term_growth_rate  # 短期增长率
    ...
    def calculate_intrinsic_value(self):
        ...
        try:
            ...
            for t in range(1, 6):  # 前 5 年
                fcf_t = fcf * ((1 + self.short_term_growth_rate) ** t)  # FCF 按短期增长率增长
                discounted_fcf = fcf_t / ((1 + wacc) ** t)  # 折现
                npv_stage_1 += discounted_fcf
                print(f"Year {t}: FCF = {fcf_t:.2f}, Discounted FCF = {discounted_fcf:.2f}")

            # 第二阶段：计算终值 (Terminal Value)
            fcf_6 = fcf * ((1 + self.short_term_growth_rate) ** 5) * (1 + self.perpetual_growth_rate)
...
if __name__ == "__main__":
    dcf = DCFModel("2330.TW", short_term_growth_rate=0.2, perpetual_growth_rate=0.025)
